...
@test "validate_host: éxito con dependencias inyectadas" {
...
CPU
  ...
  run "$REPO_ROOT/infrastructure/bin/validate_host"

  [ "$status" -eq 0 ]
  [[ "$output" == *"[OK] All requirements met"* ]]
}

@test "validate_host: detecta error cuando no se puede leer la RAM" {
  stub_dir="$TMP_WORKDIR/stubs"
  mkdir -p "$stub_dir"
...
@test "setup_project: modo dry-run no realiza cambios" {
  project_root="$TMP_WORKDIR/proyecto"
  export PROJECT_ROOT_OVERRIDE="$project_root"

  run "$REPO_ROOT/infrastructure/bin/setup_project" --dry-run
...
@test "setup_project: crea estructura mínima" {
  project_root="$TMP_WORKDIR/proyecto"
  export PROJECT_ROOT_OVERRIDE="$project_root"
  export SETUP_PROJECT_SKIP_SECRETS=1

  run "$REPO_ROOT/infrastructure/bin/setup_project"

  [ "$status" -eq 0 ]
  [[ -d "$project_root/infrastructure/bin" ]]
  [[ -d "$project_root/docs" ]]
  [[ -f "$project_root/.gitignore" ]]
  grep -q "infrastructure/config/secrets.env" "$project_root/.gitignore"
  [[ "$output" == *"[OK] Project setup complete"* ]]
}

@test "install_apache: dry-run muestra pasos" {
...
HELPER
  ...
  run "$REPO_ROOT/infrastructure/installation/install_apache.sh" --dry-run

  [ "$status" -eq 0 ]
  [[ "$output" == *"[DRY-RUN] Instalación de Apache completada"* ]]
}

@test "install_apache: propaga fallos de validación" {
...
HELPER
  ...
  run "$REPO_ROOT/infrastructure/installation/install_apache.sh"
